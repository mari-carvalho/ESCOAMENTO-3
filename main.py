...
S = # salinidade a ser fornecida pelo exemplo
Mg = # Massa a ser fornecida pelo exemplo
Pb = # Pressão de Bolha a ser fornecida pelo exemplo ????
# Tpc e Ppc serão dados na entrada da tabela - Temperatura crítica será sempre a mesma?

# Pressões e Temperatura e Referência

# dúvida: serão informadas diversas pressões na tabela e apenas uma temperatura, bem como apenas uma única temperatura crítica. As pressões críticas serão diversas para cada uma variar com cada pressão informada, ou será apenas uma que vai calcular parar todas as pressões informadas?
...
for i in range(0, len(Pvec)):   # número de linhas deve ser o mesmo do número de pressões que serão usadas
    if P > Pb:
    a_rsb[i] = (7.916 * 10 * -4) * (API ** 1.541) - (4.561 * 10 - 5) * (T ** 1.3911)  # T em F, vai calcular o parâmetro a para a linha da vez
    rsb[i,3] = (((Pb / 112.727) + 12.34) * (dg ** 0.8439) * (10 * a_rsb[i])) ** 1.73184  # P em psia e T em F, vai calular a razão de solubilidade (p/ P>Pb) e guardar na linha da vez e na coluna 3
    pb[i,2] = ((112.727 * (rsb[i,3] * 0.577421)) / ((dg ** 0.8439) * 10 * a_pb)) - 1391.051  # T em F, vai calcular a pressão de bolha e guardar na linha da vez e na coluna 2 usando a razão de solubilidade que esta na linha da vez e na coluna 3
    elif P <= Pb:
    a_rs[i] = (7.916 * 10 * -4) * (API ** 1.541) - (4.561 * 10 - 5) * (T ** 1.3911)  # T em F, vai calcular o parâmetro a para a linha da vez
    rs[i,3] = (((P[i,1] / 112.727) + 12.34) * (dg ** 0.8439) * (10 * a_rs[i])) ** 1.73184  # P em psia e T em F, vai calular a razão de solubilidade (p/ P<=Pb) e guardar na linha da vez e na coluna 3
    pb[i,2] = ((112.727 * (rs[i,3] * 0.577421)) / ((dg ** 0.8439) * 10 * a_pb)) - 1391.051  # T em F, vai calular a pressão de bolha e guardar na linha da vez e coluna 2

# Compressibilidade Isotérmica do Óleo
for i in range(0, len(Pvec)):
    if P >= Pb:
        co[i,4] = 1.705 * (10 * -7) * (rs[i,3] ** 0.69357) * (dg ** 0.1885) * (API ** 0.3272) * (T ** 0.6729) * (P[i,1] ** -0.5906)  # T em F, vai calcular a compresisbilidade isotérmica (p/ P>=Pb) e guardar na linha da vez e coluna 4 usando a razão de solubilidade que esta na linha da vez e na coluna 3 e a pressão da linha da vez e que esta na coluna 1
    elif P < Pb:
    #derivada
...
for i in range(0, len(Pvec)):
    if P > Pb:
        bob[i] = 1.0113 + (7.2046 * 10 * -5) * ((rsb[i,3] ** 0.3738) * (dg ** 0.2914 / do ** 0.6265) + 0.24626 * (T ** 0.5371)) ** 3.0936)  # T em F, usamos rsb, pois é a razão de solubilidade na pressão de bolha calculada anteriormente. Vai calcular o fato volume formação no ponto de bolha da linha da vez usando a razão de solubilidade na presão de bolha que esta na linha da vez e na coluna 3
...
for i in range(0, len(Pvec)):
    if P > Pb:
        rho_ob[i] = (62.4 * do + 0.0136 * rsb[i,3] * dg) / bob[i]   # usamos rsb e bob, pois são a razão de solubilidade e o fator volume formação do óleo na pressão de bolha calculados anteriormente. Vai calcular a massa específica na pressão de bolha para a linha da vez usando a razão de solubilidade e o fator volume formação na pressão de bolha que estão na linha da vez e nas colunas 3 e 5, respectivamente
        rho_o[i,6] = rho_ob[i] * np.exp(co * ([i,1] - Pb))   # vai calcular a massa específica (p/ P>Pb) e guardar na linha da vez e na coluna 6
    elif P <= Pb:
        rho_[i,6] = (62.4 * do + 0.0136 * rs[i,3] * dg) / bo[i,5]   # vai calcular a massa específica (p/ P<=Pb) e guardar na linha da vez e coluna 6 usando a razão de solubilidade e o fato volume formação que estão na linha da vez e nas colunas 3 e 5, respectivamente

# Viscosidade do Óleo Morto
a_uod = 10 ** (0.43 + 8.33 / API)   # vai calcular o parâmetro a
uod = 0.32 + (1.8 * 10 * 7 / API * 4.53) * ((360 / T - 260) ** a_uod)  # T em Rankine   # vai calcular a viscosidade do óleo morto
...
for i in range(,0 len(Pvec)):
    ...
    uob[i,7] = a_uob[i] * uod ** b_uob[i]   # vai calcular a viscosidade do óleo saturado (p/ P<=Pb) e guardar na linha da vez e coluna 7

# Viscosidade do Óleo Sub-Saturado
for i in range(0, len(Pvec)):
    # P >= Pb
    uo[i,8] = uob[i,7] + ( 0.001 * (P[i,1] - Pb)) *(0.024 * (uob[i,7] ** 1.6) + 0.038 * (uob[i,7] ** 0.56))   # vai calcular a viscosidade do óleo sub-saturado (p/ P>=Pb) e guardar na linha da vez e coluna 8 usando a viscosidade do óleo saturado que esta na linha da vez e coluna 7
...
for i in range(0, len(Pvec)):
    dz_dp[i] = 1 / (((0.62 - 0.23 * Tpr) * Ppr + (0.066 / (Tpr - 0.86) - 0.37) * Ppr ** 2 + (0.32 / (10 ** (9 * Tpr - 9))) * Ppr ** 6) * np.exp((((0.62 - 0.23 * Tpr) + ((0.132 / (Tpr - 0.86)) - 0.074) * Ppr) + (1.92 / (10 ** (9 * Tpr - 9))) * Ppr ** 5))) + (10 ** (0.3106 - 0.49 * Tpr + 0.1824 * Tpr ** 2) * (Ppr ** (((10 ** (0.3106 - 0.49 * Tpr + 0.1824 * Tpr ** 2)) - 1) * (0.132 - 0.32 * np.log10(Tpr)))))   # vai calcular a derivada da linha da vez
    cpr[i] = 1 / Ppr - 1 / z * dz_dp[i]   # vai calcular o parâmetro cpr da linha da vez

    cg[i,3] = cpr[i] / Ppc   # vai calcular a compressibilidade isotérmica e guardar na linha da vez e na coluna 3

# Fator Volume Formação
for i in range(0, len(Pvec)):
    bg[i,4] = 14.7 / 60 * ((z[i,2] * T) / P[i,1])  # Psc = 14.7 psia e Tsc = 60 F   # vai calular o fator volume formação e guadar na linha da vez e na coluna 4 usando o fator de compressibilidade isotérmica que esta na linha da vez e na coluna 2
...
for i in range(0, len(Pvec)):
    ...
    ug[i,6] = 10 ** - 4 * k_ug[i] * np.exp(a_ug[i])   # vai calcular a viscosidade do gás e guardar na linha da vez e na coluna 6

# ------------------------Fase Água------------------------#

# Razão de Solubilidade   e o rsw brine????
ao_rsw = (8.15839)
a1_rsw = (−6.12265 * (10 ** - 2))
a2_rsw = (1.91663 * (10 ** − 4))
a3_rsw = (−2.1654 * (10 ** −7) )
...
for i in range(0, len(Pvec)):
    ...
    Vwp[i] = (-195301 * 10 ** -9) * P[i,1] * T - (1.72834 * 10 ** -13) * (P[i,1] ** 2) * T - (3.58922 * 10 ** -7) * P[i,1] - (2.25341 * 10 ** -10) * P[i,1] ** 2  # T em F, vai calcular o parâmetro vwp da linha da vez
...
for i in range90, len(Pvec)):
    uw[i,6] = uw1 * (0.9994 + (4.0295 * 10 ** -5) * P[i,1] + (3.1062 * 10 ** -9) * P[i,1] * 2)   # vai calcular a viscosidade da água e guardar na linha da vez e na coluna 6